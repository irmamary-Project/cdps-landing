...
export async function GET(req: Request) {
  try {
    ...
    let query = supabase.from("portfolios").select("*, students!inner(nama, nis)").eq("school_id", schoolId).order("tanggal", { ascending: false }).limit(limit);
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await admin.from("portfolios").insert({
      school_id: schoolId,
      ...body,
      created_by: user.id,
    }).select().single();
